...
import { Book, Character, AnyBook } from '../types';

interface LibraryScreenProps {
  books: AnyBook[];
  selectedBook: AnyBook | null;
  storyProgress: Record<string, number>;
  onBookSelect: (book: AnyBook) => void;
  onCharacterSelect: (character: Character) => void;
  onStartStory: (book: AnyBook) => void;
  onBackToGrid: () => void;
  onCreateNovel: () => void;
}
...
const UserGeneratedBookCard: React.FC<{ book: AnyBook; progress: number; onSelect: () => void }> = ({ book, progress, onSelect }) => {
  const hasProgress = progress > 0;
  return (
    <button onClick={onSelect} className="w-full text-right group transition-transform duration-300 hover:-translate-y-2">
      <div className="w-full h-full flex flex-col rounded-2xl shadow-lg hover:shadow-2xl bg-brand-surface-dark border border-white/10 overflow-hidden">
        <div className="flex-grow flex flex-col items-center justify-center text-center p-6 bg-slate-800">
        ...
        <div className="p-4">
            {hasProgress ? (
              ...
              <p className="text-xs text-brand-text-medium font-arabic">رواية مستخدم</p>
            )}
        </div>
      </div>
    </button>
...
const CreateNovelCard: React.FC<{ onSelect: () => void }> = ({ onSelect }) => {
    return (
      <button onClick={onSelect} className="w-full text-center group transition-transform duration-300 hover:-translate-y-2">
        <div className="w-full h-full flex flex-col items-center justify-center rounded-2xl shadow-lg hover:shadow-2xl bg-brand-surface-dark/50 border-2 border-dashed border-slate-600 hover:border-amber-500 transition-colors">
            <div className="p-6">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-slate-500 group-hover:text-amber-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
                <p className="text-lg font-bold text-brand-text-medium group-hover:text-amber-500 font-arabic mt-2">أنشئ روايتك</p>
            </div>
        </div>
      </button>
    );
...
const BookScreen: React.FC<{ book: AnyBook; onCharacterSelect: (character: Character) => void; onStartStory: (book: AnyBook) => void; onBack: () => void; hasProgress: boolean; }> = ({ book, onCharacterSelect, onStartStory, onBack, hasProgress }) => {
  const [showCharacters, setShowCharacters] = useState(false);
  const isUserBook = book.isUserGenerated;
  ...
  return (
    <div className="w-full h-full p-4 sm:p-6 flex flex-col text-brand-text-light animate-fade-in">
      ...
      <div className="flex-grow overflow-y-auto space-y-8">
        ...
        <div className={`grid grid-cols-1 ${isUserBook ? 'md:grid-cols-1' : 'md:grid-cols-2'} gap-4 pt-4 max-w-2xl mx-auto`}>
        ...
        {showCharacters && !isUserBook && (
          <div className="pt-6 animate-fade-in-up max-w-2xl mx-auto">
            ...
            <div className="space-y-3">
              {(book as Book).characters.map(character => (
...
export const LibraryScreen: React.FC<LibraryScreenProps> = ({ books, selectedBook, storyProgress, onBookSelect, onCharacterSelect, onStartStory, onBackToGrid, onCreateNovel }) => {
  if (selectedBook) {
    ...
    <div className="p-4 sm:p-6 w-full h-full overflow-y-auto animate-fade-in">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6">
        ...
        {books.map((book) => {
            ...
            : <UserGeneratedBookCard key={book.id} book={book} progress={progress} onSelect={() => onBookSelect(book)} />
        })}
      </div>
    </div>
  );
