      ...
      <div className="w-full h-full flex flex-col rounded-2xl shadow-lg hover:shadow-2xl bg-brand-surface-dark border border-white/10 overflow-hidden">
        ...
        <div className="p-4">
          <div className="w-full bg-slate-700 rounded-full h-2">
            <div className="bg-gradient-crimson-amber h-2 rounded-full" style={{ width: '45%' }}></div>
          </div>
          <p className="text-xs text-brand-text-medium mt-2 font-arabic">45% مكتمل</p>
        </div>
      </div>
    </button>
  );
...
const BookScreen: React.FC<{ book: Book; onCharacterSelect: (character: Character) => void; onStartStory: (book: Book) => void; onBack: () => void }> = ({ book, onCharacterSelect, onStartStory, onBack }) => {
      ...
      <div className="flex-grow overflow-y-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 max-w-2xl mx-auto">
            <button onClick={() => onStartStory(book)} className="group relative w-full flex flex-col items-center justify-center p-6 bg-gradient-crimson-amber text-white rounded-2xl shadow-lg hover:shadow-glow-amber transition-all transform hover:-translate-y-1">
                ...
                <span className="font-bold text-xl font-arabic">ابدأ القصة</span>
            </button>
        ...
        {showCharacters && (
          <div className="pt-6 animate-fade-in-up max-w-2xl mx-auto">
            <h3 className="text-xl text-center font-bold font-arabic mb-4">اختر شخصية للدردشة</h3>
...
export const LibraryScreen: React.FC<LibraryScreenProps> = ({ books, selectedBook, onBookSelect, onCharacterSelect, onStartStory, onBackToGrid }) => {
  if (selectedBook) {
    return <BookScreen book={selectedBook} onCharacterSelect={onCharacterSelect} onStartStory={onStartStory} onBack={onBackToGrid} />;
  }
  ...
  return (
    <div className="p-4 sm:p-6 w-full h-full overflow-y-auto animate-fade-in">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6">
        {books.map((book) => (
          <BookCard key={book.id} book={book} onSelect={() => onBookSelect(book)} />
        ))}
      </div>
    </div>
  );
